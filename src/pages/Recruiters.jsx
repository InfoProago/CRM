    ...
    return (
      <Dialog open={infoOpen} onOpenChange={setInfoOpen}>
        <DialogContent className="w-[95vw] max-w-[1200px] h-[85vh] p-4">
          <DialogHeader><DialogTitle>Info • {/* inline rename */}<input className="border rounded px-2 py-1" value={rec.name} onChange={(e)=> setRecruiters(rs => rs.map(r => r.id===rec.id ? ({...r, name: e.target.value}) : r))} /></DialogTitle></DialogHeader>
          ...
          <div className="flex flex-wrap items-center gap-4 border rounded-lg p-3 mb-3">
            ...
            <div><b>Name:</b> {/* inline rename */}<input className="border rounded px-2 py-1" value={rec.name} onChange={(e)=> setRecruiters(rs => rs.map(r => r.id===rec.id ? ({...r, name: e.target.value}) : r))} /></div>
              ...
              <tbody>
                {sorted.map(r => (
                  <tr key={r.id} className="border-t">
                    <td className="p-3 font-medium text-left">{(() => { const parts = titleCase(r.name).split(" "); return (<span className="whitespace-pre-line">{parts.join("\n")}</span>); })()}</td>
                    <td className="p-3 text-center">{r._rankAcr}</td>
                    <td className="p-3 text-center">{(r._form || []).join(" - ")}</td>
                    <td className="p-3 text-center">{r._avg.toFixed(2)}</td>
