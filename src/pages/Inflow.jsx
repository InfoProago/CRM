...
const formatPhoneByCountry = U.formatPhoneByCountry || function (raw) {
  ...
  if (clean.startsWith("+352")) {
    ...
    return { ok: true, display: "+352 " + rest.replace(/(\d{3})(\d{3})(\d{3})/, "$1 $2 $3") };
  }
...
export default function Inflow({ pipeline, setPipeline, onHire }) {
  ...
  const changeFieldDebounced = debounce((keyName, id, field, value) => {
  stableUpdate((p)=>{
    p[keyName] = p[keyName].map(it => it.id===id ? ({...it, [field]: value}) : it);
  });
}, 200);
...
const stableUpdate = (updater) => {
  ...
  <td className="p-3 text-center">
    <Button size="sm" variant="outline" onClick={() => stableUpdate((p)=>{
      p[keyName] = p[keyName].map((it)=> it.id===x.id ? { ...it, calls: Math.min(3, (it.calls ?? 0)+1) } : it);
    })}>
      {x.calls ?? 0}
    </Button>
  </td>
