...
export default function Planning({ recruiters, planning, setPlanning, history, setHistory }) {
  ...
  const addZone = (ti) =>
    setDraft((d) => {
      const teams = clone(d.teams || []);
      const ez = teams[ti].extraZones || [];
      if (ez.length >= 2) return d; // Max 3 zones total (Zone1 + 2 extras)
      const count = ez.length + 2; // Zone 2 / Zone 3
  ...
  const addRow = (ti) =>
    setDraft((d) => {
      const teams = clone(d.teams || []);
      if ((teams[ti].rows?.length || 0) >= 3) return d; // Max 3 recruiters per team
  ...
  const saveDay = () => {
    ...
    for (const t of draft.teams || []) {
      for (const r of t.rows || []) {
        ...
        const sumB2 = Number(r.box2_noDisc || 0) + Number(r.box2_disc || 0);
        const sumB4 = Number(r.box4_noDisc || 0) + Number(r.box4_disc || 0);
        if (sumB2 > sc || sumB4 > sc) {
          alert("Totals cannot exceed Score: (Box 2 + Box 2*) ≤ Score and (Box 4 + Box 4*) ≤ Score.");
          return;
        }
      }
    }
        ...
        >
          ...
          <div className="grid gap-3 h-[calc(90vh-7.5rem)] overflow-y-auto pr-1 pb-2">
            {(draft?.teams || []).map((t, ti) => {
                        ...
                        <tbody>
                          {(t.rows || []).map((r, ri) => (
                            <tr key={ri} className="border-t">
                              ...
                              <td className="p-2 text-right">
                                <Input className="w-28 h-9 text-right" inputMode="numeric" value={r.game ?? ""} onChange={(e)=> setRow(ti, ri, { game: onlyNum(e.target.value) })} />
                              </td>
              ...
              <div className="mt-2 text-sm"><span className="font-semibold">Average:</span> {(() => {
                const rows = (t.rows||[]).filter(r=>r.recruiterId);
